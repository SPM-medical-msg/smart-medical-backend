...
public class NoticeServiceImpl extends ServiceImpl<NoticeMapper, Notice> implements NoticeService {
     ...
     */
    @Override
    public Result<?> selectNoticeListWithDept(Integer deptId, String title, Integer pageNum, Integer pageSize) {
        PageHelper.startPage(pageNum, pageSize);
        // 使用新的Mapper方法进行关联查询
        List<Notice> data = noticeMapper.selectByCondition(deptId, title);
        return ResultUtil.success(1, "成功", new PageInfo<>(data));
    }
     ...
     */
    @Override
    public Result<?> selectNoticeInfoWithDept(Integer id) {
        // 使用新的Mapper方法查询详情（包含科室名称）
        Notice notice = noticeMapper.selectByIdWithDeptName(id);
        return ResultUtil.success(1, "成功", notice);
    }
     ...
     */
    @Override
    public Result<?> selectNoticeListByDeptId(Integer deptId) {
        List<Notice> data = noticeMapper.selectByDeptId(deptId);
